...
base.archivesName.set("${name.split("-").let{it.subList(0, it.size-1)}.joinToString("-")}-${project["croptopia_version"]}-compat")

val environment: Map<String, String> = System.getenv()
val releaseName = "${name.split("-").let{it.subList(0, it.size-2)}.joinToString(" ") { it.capitalize() }} Compat: ${name.split("-").let { it.subList(0, it.size-1) }.last().capitalize()} ${project["croptopia_version"]}"
...
dependencies {
    ...
    modImplementation("curse.maven:croptopia-${project["croptopia_id"]}:${project["croptopia_file"]}")
}

tasks.processResources {
    duplicatesStrategy = DuplicatesStrategy.INCLUDE

    inputs.property("version", "${project.version}-${project["croptopia_version"]}")

    from(sourceSets["main"].resources.srcDirs) {
        include("fabric.mod.json")
        expand(mutableMapOf("version" to "${project.version}-${project["croptopia_version"]}"))
    }
